"""
UI components for the chatbot interface (in-page sidebar style)
"""
...
def render_session_controls(sessions_dict, current_session_id):
    """Render the session controls on the left column"""
    ...
    if st.session_state.new_chat_mode:
        st.markdown("#### 📝 Name your chat")
        st.session_state.temp_new_title = st.text_input(
            "Enter chat title:",
        ...
        if col1.button("✅ Create"):
            ...
            if not title:
                st.error("Please enter a non-empty title or click Skip.")
            else:
    ...
    if sessions_dict:
        ...
        for sid, s in sorted(sessions_dict.items(), key=lambda kv: kv[0], reverse=True):
            if st.button(f"{s['title']}", key=f"session_{sid}"):
            ...
            "messages": [
                {
                    ...
                    "model_id": m.get("model_id", DEFAULT_MODEL)
